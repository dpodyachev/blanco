<?php
header("Location: /stalker_portal/server/administrator/m3u.php?".$_SERVER["QUERY_STRING"],TRUE,307);
...
//Mysql::getInstance()->query('select * from epg');
...
echo $row->col1 . ' : ' . $row->col2 . '<br />';
        ...
        $raw = Mysql::getInstance()->query('select descr from epg where ch_id=2 and id=89')->all();
//='.$this->id.' and (rent_end_date>NOW() OR rent_date=rent_end_date)')->all();
        foreach ($raw as $rt){
            echo ($rt['descr']);
        }
