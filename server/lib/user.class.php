...
class User implements \Stalker\Lib\StbApi\User
{
    ...
    /**
     * update itv subscription 
     * @param int $userId
     * @param int $tarifPlan
     * @return mixed
     */
    public static function updateItvSubscription($userId, $tarifPlan)
    {
        $externalId = (int) Mysql::getInstance()
            ->from('services_package')
            ->where(array('external_id' => (int) $tarifPlan))
            ->get()
            ->first('id');
        $ids = Mysql::getInstance()
            ->from('service_in_package')
            ->where(array('package_id' => (int) $externalId))
            ->get()
            ->all('service_id');

        return ItvSubscription::updateByUids(array($userId), array('sub_ch' => $ids));
    }
}